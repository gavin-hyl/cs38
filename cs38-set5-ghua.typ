...
- The ordering of the subproblems are as follows.
  ...
  Then, each element would only depend on three elements in the row before it (indexed $i-1$).
...
We know, by definition, that the optimal cost is given by the minimum of the elements in $M[m, n]$. Call that element $E_min [m, n]$ ($E_min [i, j]$ in general).
Add the pairing represented by $E_min [m, n]$ to the optimal matching, which is
- $(x[m], -)$ if $E_min [m, n] = E[m, n F, T]$
- $(x[m], y[n])$ if $E_min [m, n] = E[m, n, F, F]$
- $(-, y[n])$ if $E_min [m, n] = E[m, n, T, F]$


The value of $E_min [i, j]$ is generated by the minimum of three elements, in the matrix elements $M[i-1, j], M[i, j-1], M[i-1, j-1]$ separately.
We can backtrack into one of those three elements, generating a new $E_min [i', j']$, which corresponds to the previous element in the optimal matching, and add the pairing represented by $E_min [i', j']$ to the optimal matching.
This effectively reverses the process of constructing $E_min [i, j]$, which builds the optimal matching.
This process continues until we reach the base case $E[0, 0]$.
